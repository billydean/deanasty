...
function Annals({year, people, events}: {year: Year, people: People, events: Events}) {
    ...
    return (
        ...
        people.map(each => (
             <div key={each.id}>
                 {each.name} is { year.current - each.birth_year} years old. They will die in the year {each.old_year}. They are { each.alive ? `alive` : `dead`}. Their fertility level is: { each.fertility }.
             </div>
         ))
        } 
        </div>
