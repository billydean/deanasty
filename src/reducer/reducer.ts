import { Action, NewsItem, State } from "../types";
import { foundHouse } from "../utils/House";
import { Person } from "../utils/Person";
import { ReducerFunction } from "../utils/ReducerFunction";
import { firstTitle } from "../utils/Title";
...
export function reducer (state: State, action: Action): State {
    switch(action.type) {
        ...
        case 'INCREMENT_YEAR':
            ...
            const { updated_dead, updated_living, news_items, updated_houses, updated_parents, updated_titles } = ReducerFunction(state.year.current,state.living_people,state.dead_people, state.houses, state.parents, state.titles, current_year_events, state.contagions)
